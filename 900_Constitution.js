...
var COMPLIANCE_OS_CONSTITUTION = {
  ...
  changelog: [
    ...
    {
      date: '2026-09-15',
      change: 'ADR-005 正式 CLOSED（Governance-only 变更，127/125/142/108/999 均未改动）：Accuracy Gate 2 完整验证链完成——(1) Same-file repeatability：同一个锁定的 drive_file_id（1fUrux2zoQgvKe0DvsPrR5Xma9pxa57yA）10 次独立真实 Gemini 呼叫，statement period、7 个 daily printed subtotal、每日订单数、70/70 独立重算 daily arithmetic、8PRUR5AGXAQRAV 历史案例逐字段、35 笔 Sekaligus 的 order_ids_raw/and_more_count 全部 10/10 一致，request prompt hash/extractor id/finish reason/promptTokenCount 亦一致；(2) 独立 source-grounded 验证：针对原始 PDF（2026-W01.pdf）做了不依赖 Gemini 输出本身的核对——statement period、七个 daily subtotal、weekly「Pendapatan bersih penghantaran」RM1,297.60、8PRUR5AGXAQRAV 全部对上；此前唯一悬而未决的 2 笔订单 ID 字符级分歧（A-8QHEGDFGX8OKAV、A-8QJRNFPWWTKQAV）已直接对 PDF 原文确认——是读到印刷字符本身，不是采用多数决推断。closure 范围明确限定：只确认「锁定的 W01 fixture，在 file 引用固定的前提下，重复真实呼叫能可靠得出正确的 statement period、checksum、与已知订单案例」——不代表 Gemini 全面准确、不代表 Gemini 不会 hallucinate、不代表未来任何 PDF 都会正确抽取、不代表 production 的 file-identity 风险已完全排除。历史上出现过 9 个不同 drive_file_id 这件事维持未解决、独立于本次 closure：只confirmed 这批验证用的是同一个 hardcode 的 fixture ID、且这份 observation harness 本身没有 runtime override 路径；历史上那 9 个值具体怎么产生的、哪个 session、production 的 document-import 路径是否有对应风险，均 not confirmed，不定性为人为失误、test harness 操作痕迹，或已排除于 production 之外。完整证据见 Gate2_ReClose_Evidence_Proposal_2026-09-10.md 与 Gate2_Independent_Source_Verification_v2_2026-09-14.md（评估文件，未纳入本 repo）。900/901 其余内容、Architecture Freeze 均未变动或重开',
      approvedBy: 'Steven'
    },
    {
      date: '2026-09-15',
      change: 'Gate 2 CLOSED 之后的下一步：Production Wiring Slice——把已经验证过的 142_DailyOrderAllocation.js 接上生产流程（ADR-004 范围内的 Daily_Allocation 表，不含 Order_Allocation/Monthly_Allocation，那两张表仍是未开始的下一阶段）。采用 Separate Execution Boundary 架构（正式确认，见 ArchitectureDecisionConfirmation_2026-09-15.md）：Execution A（110_DocumentImport.js）完全不变，仍然只做 PDF→weekly extraction→Verified Income persistence；Execution B 是新增的独立入口 consoleRunDailyAllocation_/consoleRunDailyAllocation（170_OperatorConsole.js），靠 verified_income_id 定位既有 Verified_Income 记录、呼叫既有 142 API（runGeminiOrderExtractionWithFallback_/writeDailyAllocationBatch_，内部逻辑一行未改）；Execution C 是 160_MonthlyProjection.js 新增的消费逻辑，跨月周若已有 Fully_Allocated 的 Daily_Allocation，订单收入按 allocation_date 正确分月计入 net_delivery_income/net，Insentif/Tip/Bayaran lain-lain 等非订单收入明确记在新增的 unallocated_non_order_income（不猜日期、不按比例分摊、不平均分配、不强行塞进某个月）。三个 execution 刻意分开、不在同一次 GAS execution 内完成，理由是 Gate 2 真实数据显示 extractOrders() 单次可达 289 秒，跟 110 现有流程堆叠在一起有真实撞 GAS 360 秒执行上限的风险，不是为了减少 Gemini 呼叫次数。实作过程中另外发现并修复：142 与 160 之间一个新增的 circular require（改成 call-time 才 require，GAS 环境本来就不受影响）、142 期待的 verifiedIncomeContext 实际形状（camelCase、期间拆解成 {year,month,day}）跟 Verified_Income 自己栏位形状不同，这个转换之前从未存在过。161/171 新增测试全部通过，16 个测试文件全套回归无新增失败（143/195 既有的 5+2 项失败逐字比对确认跟原始未改动的 zip 完全相同，属于这次改动之前就存在的问题，不在这次授权范围内未处理）。**这次改动只完成本地/Node 测试环境的验证，尚未在真实 GAS 环境跑过，状态明确是 PENDING，不得视为 production ready**——完整实作细节、测试证据、待 Steven 在真实环境确认的清单，见 ProductionWiringSlice_Implementation_2026-09-15.md。110/142/125/127/108/999 均未改动；ADR-005 未重开、未修改。历史 9-file-id 溯源问题维持独立于本次改动、未解决状态，不因这次进展而被视为已排除。',
      approvedBy: 'Steven'
    }
  ],
