...
function buildSheetSchemas_() {
  return [
    ...
    },
    {
      ...
      textColumns: ['income_id', 'period', 'currency', 'source', 'origin_platform', 'status', 'verified_at', 'source_document_id', 'extractor_id', 'period_start', 'period_end']
      // period_start/period_end 是 "YYYY-MM-DD" 字符串——跟 verified_at
      // 同一个理由，一定要留在 textColumns，不然 Sheets 会把这种看起来
      // 像日期的字符串静默转成日期序列值（这正是 2026-08-22 撞到的那个
      // 真实 GAS 崩溃的成因之一——106_Utils.js 的 normalizeIsoDateString_
      // 是即使 textColumns 设置生效前／人为改了格式，仍然读到原生 Date
      // 物件时的第二道防线，不是取代这里）
